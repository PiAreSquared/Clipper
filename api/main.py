...
from exceptions import FileLimitExceededException, FileTypeNotSupportedException
...
@app.exception_handler(FileTypeNotSupportedException)
async def file_type_not_supported_exception_handler(request, exc: FileTypeNotSupportedException):
...
async def upload_file(request: Request):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=f"Missing required headers, or formatted incorrectly. {dir(request.headers)}")
    
    if content_type not in ALLOWED_FILE_TYPES:
        raise FileTypeNotSupportedException(content_type=content_type)
    
    try:
        filepath = f"/tmp/{uuid.uuid4()}-{filename}"
        size_validator = MaxFileSizeValidator(MAX_FILE_SIZE)
        ...
        raise HTTPException(status_code=status.HTTP_499_CLIENT_CLOSED_REQUEST, detail="Client disconnected while uploading file.")
    except FileLimitExceededException as e:
        raise HTTPException(status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE, detail=str(e))
    except ValidationError as e:
        raise HTTPException(status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f'There was an error processing the file. Please try again. {e}')
